...
describe("HybridVFS", () => {
  ...
  describe("stat", () => {
    ...
    it("should throw for non-existent host paths", () => {
      expect(() => vfs.stat("/mnt/host/nonexistent.txt")).toThrow(
  ...
  describe("read", () => {
    ...
    it("should throw for non-existent files", () => {
      expect(() => vfs.read("/mnt/host/nonexistent.txt")).toThrow(
        "no such file or directory",
      );
    });
  ...
  describe("write", () => {
    ...
    it("should throw when writing to directories", () => {
      expect(() => vfs.write("/mnt/host/subdir", "content")).toThrow(
        "not a file",
      );
    });
    ...
    it("should throw when parent directory does not exist", () => {
      expect(() =>
        vfs.write("/mnt/host/nonexistent/file.txt", "content"),
  ...
  describe("remove", () => {
    ...
    it("should throw when removing non-empty directory without recursive", () => {
      expect(() => vfs.remove("/mnt/host/subdir")).toThrow(
        "directory not empty",
      );
    });
    ...
    it("should not allow removing mount point", () => {
      expect(() => vfs.remove("/mnt/host")).toThrow(
  ...
  describe("copy", () => {
    it("should copy host files within host", () => {
      ...
      expect(
        fs.readFileSync(path.join(testDir, "test-copy.txt"), "utf-8"),
      ).toBe("Hello World");
    });
    ...
    it("should copy virtual file to host filesystem", () => {
      ...
      expect(
        fs.readFileSync(path.join(testDir, "virtual-copy.txt"), "utf-8"),
    ...
    it("should copy host directories recursively", () => {
      ...
      expect(
        fs.existsSync(path.join(testDir, "subdir-copy", "nested.txt")),
      ).toBe(true);
    });
  });
  ...
  describe("move", () => {
    ...
    it("should move virtual file to host filesystem", () => {
      ...
      expect(
        fs.readFileSync(path.join(testDir, "virtual-moved.txt"), "utf-8"),
      ).toBe("Virtual content");
    });
  });
  ...
  describe("read-only mode", () => {
    ...
    it("should block writing files", () => {
      expect(() => readOnlyVfs.write("/mnt/host/new.txt", "content")).toThrow(
        "read-only",
      );
    });
    ...
    it("should block removing files", () => {
      expect(() => readOnlyVfs.remove("/mnt/host/test.txt")).toThrow(
        "read-only",
      );
    });

    it("should block appending to files", () => {
      expect(() => readOnlyVfs.append("/mnt/host/test.txt", " more")).toThrow(
        "read-only",
      );
    });
  });
