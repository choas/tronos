  ...
  describe("command parsing", () => {
    ...
    it("should return validation error for create without name", async () => {
      ...
      expect(result.stderr).toContain(
        "'create' command requires a program name",
      );
    });
    ...
    it("should return validation error for explain without file", async () => {
      ...
      expect(result.exitCode).toBe(1);
      expect(result.stderr).toContain(
        "'explain' command requires a target file",
      );
    });
...
`,
      ...
      const result = await ai(
        ["create", "hello", "a", "simple", "hello", "world", "program"],
        context,
      );
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          content: [
            {
              type: "text",
              text: 'This program outputs "Test" to the terminal.',
            },
          ],
          usage: { input_tokens: 100, output_tokens: 50 },
        }),
...
`,
      ...
      const result = await ai(
        ["edit", "/test.trx", "change", "output", "to", "Updated"],
        context,
      );
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          content: [
            {
              type: "text",
              text: "The ls command lists files and directories.",
            },
          ],
...
`,
    ...
    it("should resolve program name without path or extension to /bin/*.trx", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          content: [
            { type: "text", text: "This program counts down from a number." },
          ],
          usage: { input_tokens: 100, output_tokens: 50 },
        }),
      });
