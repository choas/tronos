...
describe("createSandboxTerminalAPI", () => {
  ...
  test("provides args as read-only copy", () => {
    ...
    const api = createSandboxTerminalAPI(ctx, args, async () => ({
      stdout: "",
      stderr: "",
      exitCode: 0,
    }));
...
async function main(t) {
  ...
  t.writeln("File created successfully");
    ...
    expect(vfs.read("/home/tronos/created-by-named.txt")).toBe(
      "Created by named function",
    );
  });
  ...
  beforeEach(async () => {
    ...
    ctx = {
      ...
      terminal: {
        ...
        getCursor: () => ({ x: 0, y: 0 }),
        onKey: (
          cb: (key: { key: string; domEvent: KeyboardEvent }) => void,
        ) => {
          mockKeyCallbacks.push(cb);
          return {
            dispose: () => {
              mockKeyCallbacks = mockKeyCallbacks.filter((c) => c !== cb);
            },
          };
        },
        onData: (cb: (data: string) => void) => {
          ...
          return {
            dispose: () => {
              mockDataCallbacks = mockDataCallbacks.filter((c) => c !== cb);
            },
          };
        },
        hasInput: () => false,
...
describe("package config access in sandbox", () => {
  ...
  test("t.config.get returns config value for installed package", () => {
    ...
    const api = createSandboxTerminalAPI(
      ctx,
      [],
      ...
      JSON.stringify([
        {
          ...
          config: [{ key: "name", type: "string", description: "Your name" }],
        },
      ]),
    );
    vfs.write(
      "/etc/tpkg/greeting/config.json",
      JSON.stringify({
        name: "Alice",
      }),
    );
...
describe("getPackageNameForExe and getMissingRequiredConfig", () => {
  ...
  test("getPackageNameForExe returns package name for installed exe", async () => {
    const { getPackageNameForExe } =
      await import("../src/engine/builtins/tpkg");
