...
describe("aiosFetch", () => {
  ...
  describe("CORS error handling", () => {
    it("should enhance CORS error messages", async () => {
      global.fetch = vi
        .fn()
        .mockRejectedValue(new Error("CORS policy blocked the request"));
    ...
    it("should enhance cross-origin error messages", async () => {
      global.fetch = vi
        .fn()
        .mockRejectedValue(new Error("Cross-origin request blocked"));
    ...
    it("should handle blocked error messages", async () => {
      global.fetch = vi
        .fn()
        .mockRejectedValue(new Error("Request was blocked"));
  ...
  describe("generic error handling", () => {
    it("should re-throw non-CORS errors", async () => {
      ...
      await expect(aiosFetch("https://example.com")).rejects.toThrow(
        "Timeout exceeded",
      );
    });
    ...
    it("should re-throw non-Error objects", async () => {
      ...
      await expect(aiosFetch("https://example.com")).rejects.toBe(
        "string error",
      );
    });
  });
