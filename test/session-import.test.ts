...
import {
  ...
  isValidDiskImage,
  isYamlContent,
  serializeDiskImageToYaml,
  mergeSession,
  formatMergeResult,
  detectMergeConflicts,
  diffDiskImage,
  recordImportHistory,
} from "../src/engine/builtins/session";
...
describe("parseDiskImage", () => {
  ...
  it("should throw on invalid JSON", () => {
    expect(() => parseDiskImage("not valid json {")).toThrow(
      "Invalid JSON format",
    );
  });
  ...
  it("should throw on valid JSON but invalid DiskImage structure", () => {
    ...
    expect(() => parseDiskImage(JSON.stringify(invalidStructure))).toThrow(
  ...
  it("should handle YAML with multi-line strings", () => {
    ...
    expect(result.files["/test.txt"].content).toBe(
      "Line 1\nLine 2\nLine 3\nLine 4",
    );
  });
...
describe("serializeDiskImageToYaml", () => {
  ...
  it("should handle file content with special characters", () => {
    ...
    expect(parsed.files["/code.trx"].content).toContain(
      "async function main(t)",
    );
...
describe("session env/aliases/history import", () => {
  ...
  it("should preserve complex aliases", () => {
    ...
    expect(parsed.session.aliases["complex"]).toBe(
...
describe("session import --merge command", () => {
  ...
  it("should reject multiple conflict resolution flags", async () => {
    const result = await session(
      ["import", "--merge", "--overwrite", "--skip"],
      context,
    );

    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain(
      "Only one of --overwrite, --skip, or --interactive",
    );
  });
...
describe("mergeSession", () => {
  ...
  it("should create directories as needed", async () => {
    ...
    expect(vfs.readSync("/home/tronos/deep/nested/dir/file.txt")).toBe(
      "nested content",
    );
  });
});
...
describe("recordImportHistory", () => {
  ...
  it("should create import history entry for merge operation", async () => {
    ...
    try {
      const entry = await recordImportHistory(
        "session-456",
        diskImage,
        false,
        mergeResult,
