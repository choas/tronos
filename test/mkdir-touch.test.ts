...
describe("mkdir builtin", () => {
  ...
  it("should support -p flag for parent creation", async () => {
    const vfs = new InMemoryVFS();
    await vfs.init();
    const result = await mkdir(["-p", "/path/to/nested/dir"], {
      stdin: "",
      env: {},
      vfs,
    });
  ...
  it("should fail without parent directories and no -p flag", async () => {
    const vfs = new InMemoryVFS();
    await vfs.init();
    const result = await mkdir(["/nonexistent/path"], {
      stdin: "",
      env: {},
      vfs,
    });
...
describe("touch builtin", () => {
  ...
  it("should update timestamp of existing file", async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(vfs.stat("/existing.txt").meta.updatedAt).toBeGreaterThan(
      originalTime,
    );
  });
  ...
  it("should create multiple files", async () => {
    const vfs = new InMemoryVFS();
    await vfs.init();
    const result = await touch(["file1.txt", "file2.txt"], {
      stdin: "",
      env: {},
      vfs,
    });
  ...
  it("should fail when parent directory does not exist", async () => {
    const vfs = new InMemoryVFS();
    await vfs.init();
    const result = await touch(["/nonexistent/file.txt"], {
      stdin: "",
      env: {},
      vfs,
    });
