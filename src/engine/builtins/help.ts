...
> = {
  ...
  echo: {
    usage: "echo [string...]",
    description:
      "Display a line of text. Supports escape sequences like \\n, \\t.",
    examples: ["echo hello", 'echo "Hello World"', 'echo -e "line1\\nline2"'],
  },
  mkdir: {
    usage: "mkdir [-p] directory...",
    description:
      "Create directories. Use -p to create parent directories as needed.",
  ...
  rm: {
    usage: "rm [-r] [-f] file...",
    description:
      "Remove files or directories. Use -r for recursive, -f for force.",
  ...
  history: {
    usage: "history [count]",
    description:
      "Display command history. Optionally limit to last N commands.",
  ...
  export: {
    ...
    description:
      "Set environment variables. Without arguments, lists all exported variables.",
  ...
  alias: {
    ...
    description:
      "Define or display aliases. Without arguments, lists all aliases.",
  ...
  type: {
    usage: "type command...",
    description:
      "Display information about command type (builtin, alias, or file)",
    examples: ["type ls", "type ll cd"],
  },
  help: {
    usage: "help [command]",
    description:
      "Display help information. Without arguments, lists all commands.",
    examples: ["help", "help ls", "help cd"],
  },
  man: {
    usage: "man [command]",
    description:
      "Display detailed manual pages for commands. Includes sections for NAME, SYNOPSIS, DESCRIPTION, OPTIONS, EXAMPLES.",
    examples: ["man ls", "man grep", "man man"],
  },
  config: {
    usage: "config [show|set <key> <value>|reset|ui]",
    description:
      'Configure AI settings. Use "config show" to display current settings, "config set" to change values.',
    examples: [
      "config show",
      "config set apiKey sk-...",
      "config set provider anthropic",
      "config set accept-terms true",
      "config reset",
    ],
  },
  tpkg: {
    ...
    description:
      "TronOS Package Manager - install, update, and manage packages from repositories.",
    examples: [
      "tpkg update",
      "tpkg search weather",
  ...
  timewarp: {
    ...
    description:
      "File version control - list, show, revert, diff, save versions and manage branches.",
    examples: [
  ...
  feedback: {
    usage:
      "feedback <message> | feedback list | feedback show <id> | feedback clear",
    description:
      "Submit and manage user feedback. Feedback is stored in the /feedback folder.",
    examples: [
  ...
  update: {
    usage:
      "update [--apply] [--skip|--overwrite] [--rollback] [--history] [--dry-run]",
    description:
      "TronOS system update mechanism. Upgrades system files while preserving user changes via timewarp versioning.",
    examples: [
      "update",
      "update --apply",
      "update --apply --skip",
...
export const help: BuiltinCommand = async (args, context) => {
  ...
  if (args.length === 0) {
    ...
    if (termWidth < NARROW_SCREEN_THRESHOLD) {
      ...
      output.push(
        "  Environment variables: TRONOS_API_KEY, TRONOS_AI_PROVIDER",
      );
    }
