...
import { getAgentRuntime, parseTrigger, type AgentTrigger } from '../../agents/runtime';
...
export const agent: BuiltinCommand = async (
  ...
  if (args.length === 0) {
    return {
      stdout: '',
      stderr: 'Usage: agent <start|list|status|log|suspend|resume|kill|load> [args...]\n' +
        '  agent start <name> "<goal>" [--read <globs>] [--write <globs>] [--every <interval>]\n' +
  ...
  switch (subcommand) {
    case 'start': {
      const name = args[1];
      if (!name) {
        return { stdout: '', stderr: 'Usage: agent start <name> "<goal>" [options...]\n', exitCode: 1 };
      }
      ...
      try {
        ...
        return { stdout: '', stderr: `Failed to start agent: ${msg}\n`, exitCode: 1 };
      }
    }
    ...
    case 'list': {
      ...
      const lines = ['ID   PID   NAME                STATUS     TRIGGER       GOAL'];
      for (const a of agents) {
        const statusColor = a.status === 'running' ? '\x1b[32m' :
          a.status === 'error' ? '\x1b[31m' :
          a.status === 'suspended' ? '\x1b[33m' : '';
        const resetColor = statusColor ? '\x1b[0m' : '';
        const triggerStr = a.trigger.type + (a.trigger.value ? `(${a.trigger.value})` : '');
        const goalStr = a.goal.length > 40 ? a.goal.substring(0, 37) + '...' : a.goal;
        lines.push(
    ...
    case 'status': {
      const id = args[1];
      if (!id) return { stdout: '', stderr: 'Usage: agent status <id>\n', exitCode: 1 };

      const a = runtime.getAgent(id);
      if (!a) return { stdout: '', stderr: `Agent not found: ${id}\n`, exitCode: 1 };
    ...
    case 'log': {
      const id = args[1];
      if (!id) return { stdout: '', stderr: 'Usage: agent log <id>\n', exitCode: 1 };
    ...
    case 'suspend': {
      const id = args[1];
      if (!id) return { stdout: '', stderr: 'Usage: agent suspend <id>\n', exitCode: 1 };
      try {
    ...
    case 'resume': {
      const id = args[1];
      if (!id) return { stdout: '', stderr: 'Usage: agent resume <id>\n', exitCode: 1 };
      try {
    ...
    case 'kill': {
      const id = args[1];
      if (!id) return { stdout: '', stderr: 'Usage: agent kill <id>\n', exitCode: 1 };
      try {
    ...
    case 'load': {
      ...
      if (!filePath) return { stdout: '', stderr: 'Usage: agent load <path>\n', exitCode: 1 };
      if (!context.vfs) return { stdout: '', stderr: 'Filesystem not available\n', exitCode: 1 };
      ...
      try {
        ...
        const { parseAgentManifest, startFromManifest } = await import('../../agents/manifest');
        const manifest = parseAgentManifest(source);
        if (!manifest.success) {
          return { stdout: '', stderr: `Parse error: ${manifest.error}\n`, exitCode: 1 };
        }
        ...
        const msg = err instanceof Error ? err.message : String(err);
        return { stdout: '', stderr: `Failed to load agent: ${msg}\n`, exitCode: 1 };
      }
    }

    default:
      return { stdout: '', stderr: `agent: unknown subcommand: ${subcommand}\n`, exitCode: 1 };
  }
};
