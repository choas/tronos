...
function formatTimestamp(filename: string): string {
  ...
  const match = filename.match(
    /feedback-(\d{4})-(\d{2})-(\d{2})_(\d{2})-(\d{2})-(\d{2})/,
  );
  if (match) {
...
export const feedback: BuiltinCommand = async (args, context) => {
  ...
  if (subcommand === "list") {
    ...
    const feedbackFiles = entries.filter((e: string) =>
  ...
  if (subcommand === "show") {
    ...
    const matchingFile = feedbackFiles.find(
      (f: string) =>
  ...
  const content = [message.trim()].join("\n");

  vfs.write(filePath, content);

  // Send feedback to backend API (fire-and-forget, don't block on failure)
  const feedbackURL =
    import.meta.env.VITE_TRONOS_FEEDBACK_URL ||
    "https://feedback.tronos.dev/api/feedback";
  try {
    fetch(feedbackURL, {
      method: "POST",
