...
export const event: BuiltinCommand = async (
  ...
  if (args.length === 0) {
    return {
      stdout: '',
      stderr: 'Usage: event <watch|list|unwatch|log> [args...]\n' +
        '  event watch <path> <event-type> --call <command>\n' +
        '  event list\n' +
        '  event unwatch <id>\n' +
        '  event log [limit]\n',
  ...
  switch (subcommand) {
    case 'watch': {
      ...
      while (i < args.length) {
        if (args[i] === '--call' && i + 1 < args.length) {
          ...
          const types: OSEventType[] = [
            'file-changed', 'context-changed', 'agent-action',
            'mcp-result', 'session-start', 'network-change',
          ];
      ...
      if (pathPattern) pattern.path = pathPattern;
    ...
    case 'list': {
      ...
      if (subs.length === 0) {
        return { stdout: 'No active event subscriptions.\n', stderr: '', exitCode: 0 };
      }
    ...
    case 'unwatch': {
      const id = args[1];
      if (!id) {
        return { stdout: '', stderr: 'Usage: event unwatch <id>\n', exitCode: 1 };
      }
