...
function handleList(
  ...
  if (jobs.length === 0) {
    return {
      stdout: 'No cron jobs scheduled. Use "cron add" to create one.',
      stderr: "",
      exitCode: 0,
    };
  }
...
function handleLog(
  ...
  if (entries.length === 0) {
    ...
    return {
      stdout: `No execution history${suffix}.`,
      stderr: "",
      exitCode: 0,
    };
  }
...
async function handleEdit(
  ...
  if (args.length >= 3) {
    ...
    if (schedError) {
      return {
        stdout: "",
        stderr: `cron: invalid schedule: ${schedError}`,
        exitCode: 1,
      };
    }
    ...
    return {
      stdout: `Updated cron job ${id}: ${newSchedule} ${newCommand}`,
      stderr: "",
      exitCode: 0,
    };
  }
...
async function handleCopy(
  ...
  if (context.vfs) {
    try {
      ...
      return {
        stdout: `Copied to clipboard: ${definition}`,
        stderr: "",
        exitCode: 0,
      };
    } catch {
      return {
        stdout: definition,
        stderr: "cron: clipboard not available, definition printed above",
        exitCode: 0,
      };
    }
  }
...
async function handlePaste(
  ...
  if (!context.vfs) {
    return {
      stdout: "",
      stderr: "cron: filesystem not available",
      exitCode: 1,
    };
  }
  ...
  try {
    ...
    return {
      stdout: "",
      stderr: "cron: could not read clipboard",
      exitCode: 1,
    };
  }
  ...
  if (!command) {
    return {
      stdout: "",
      stderr: "cron: no command found in clipboard content",
      exitCode: 1,
    };
  }
  ...
  if (schedError) {
    return {
      stdout: "",
      stderr: `cron: invalid schedule from clipboard: ${schedError}`,
      exitCode: 1,
    };
  }
